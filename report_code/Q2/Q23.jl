...
for (i, Io) in enumerate(Io_values)
    I_ext_L(x,t) = I_ext_2(x,t,-Io)
    I_ext_R(x,t) = I_ext_2(x,t,Io)
    # simulate the network activity
    spikes_L, spikes_R = simulate_networks(h_init_L, h_init_R, x_i, N, delta_t, n, R, tau, I_ext_bool, J, alpha, beta, ro, theta, I_ext_L, I_ext_R, true)
